...
{showNumeros && (
    ...
    {showGalerieOfficielle && <GalerieOfficielle onClose={() => setShowGalerieOfficielle(false)} />}
      ...
      <main style={{ maxWidth: 900, margin: "0 auto", padding: "18px 24px 120px", position: "relative", zIndex: 20 }}>        
{activeTab === "accueil" && (
          <div style={{ opacity: visible ? 1 : 0, transition: "opacity 0.8s ease" }}>
{/* Compte à rebours */}
            <div style={{ marginBottom: 18 }}>
              <div style={{ textAlign: "center", marginBottom: 10 }}>
                <span style={{ fontSize: 12, color: COLORS.jaune, letterSpacing: 3, textTransform: "uppercase" }}>
                  ⏳ Compte à rebours
                </span>
              </div>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 10 }}>
                {Object.entries(countdown).map(([unit, val]) => (
                  <div key={unit} style={{
                    ...
                    <div style={{
                      fontSize: "clamp(20px, 5vw, 34px)", fontWeight: "bold",
                      color: COLORS.jaune, lineHeight: 1, fontVariantNumeric: "tabular-nums",
                    }}>{String(val).padStart(2, "0")}</div>
                    <div style={{ fontSize: 10, color: "rgba(240,234,214,0.5)", textTransform: "uppercase", letterSpacing: 1.5, marginTop: 4 }}>
                      {unit}
                    </div>
                  </div>
                ))}
              </div>
            </div>          {/* Rangée haut : Réalisations | Président */}
...
<div style={{
        ...
        background: "linear-gradient(135deg, rgba(0,60,40,0.97), rgba(10,20,10,0.97))",
        ...
        <div onClick={() => {
          ...
          <span style={{ color: COLORS.jaune, fontSize: 11, fontWeight: "bold" }}>Accueil</span>
        </div>
...
{showBanniereApp && (
      <div style={{
       ...
       <div style={{ flex: 1, color: "#fff", fontSize: 12, lineHeight: 1.3 }}>
        ...
        <a href="https://play.google.com/store/apps/details?id=com.makokou.liberation"
          target="_blank"
          rel="noreferrer"
          style={{
...
{!showAssistant && (
      <div
        ...
        style={{
          position: "fixed",
          bottom: "max(90px, env(safe-area-inset-bottom, 24px) + 80px)",
          left: 20,
...
{(activeTab !== "accueil" || showHotels || showRestaurants || showNumeros || showBioPresidente || showBioPresident || showQuiz || showAssistant || showGalerieCollab) && (
      ...
      >
        ...
        <span style={{ color: "#fff", fontSize: 12, fontWeight: "bold" }}>Retour</span>
      </div>
    )}  </div>
  );
}
