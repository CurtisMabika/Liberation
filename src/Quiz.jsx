...
export default function Quiz({ onClose }) {
  ...
  const startQuiz = () => {
    ...
    const shuffled = shuffle(quizQuestions).slice(0, 10);
  ...
  const answer = (index) => {
    ...
    setTimeout(() => {
      if (current + 1 < questions.length) {
        ...
        finishQuiz(index === questions[current].correct ? score + 1 : score);
      }
    }, 1000);
  };
      ...
      <div style={{ ...cardStyle, maxWidth: 420, width: "100%", maxHeight: "85vh", overflowY: "auto" }}>
        ...
        {step === "intro" && (
          <div>
            <p style={{ fontSize: 14, opacity: 0.85, marginBottom: 16 }}>
              10 questions sur la province de l'Ogooué-Ivindo. Entre ton pseudo pour commencer !
            </p>
            <input
              value={pseudo}
            ...
            <button
              ...
              style={{ width: "100%", padding: 14, borderRadius: 8, border: "none", background: COLORS_OR, color: "#1a2e1a", fontWeight: "bold", fontSize: 16, cursor: "pointer", opacity: pseudo.trim() ? 1 : 0.5 }}
            >
              Commencer le quiz
            </button>
            <button
        ...
        {step === "result" && (
          <div style={{ textAlign: "center" }}>
            ...
            <button onClick={() => setStep("intro")} style={{ width: "100%", padding: 12, borderRadius: 8, border: "none", background: COLORS_OR, color: "#1a2e1a", fontWeight: "bold", marginTop: 10, cursor: "pointer" }}>
              Rejouer
            </button>
          </div>
        )}
        ...
        {step === "leaderboard" && (
          <div>
            ...
            <button onClick={() => setStep("intro")} style={{ width: "100%", padding: 12, borderRadius: 8, border: "none", background: COLORS_OR, color: "#1a2e1a", fontWeight: "bold", marginTop: 16, cursor: "pointer" }}>
              Retour
            </button>
          </div>
        )}
